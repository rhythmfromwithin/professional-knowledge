...
> 原文: [https://medium.com/airbnb-engineering/how-we-knew-covid-was-over-and-what-our-models-had-to-unlearn-c606b9bdb0ab?source=rss----53c7c27702d5---4](https://medium.com/airbnb-engineering/how-we-knew-covid-was-over-and-what-our-models-had-to-unlearn-c606b9bdb0ab?source=rss----53c7c27702d5---4)

#### *When we retrain, when we rebuild, and when we leave a model alone.*

![A wooden trail signpost against a bright blue sky with clouds, displaying multiple Italian hiking directional signs marked with trail number 708 and 701, listing destinations such as Sentiero 708B, Case Zampini, Rifugio di Monte Scavo, Rifugio Galvarina, and Rifugio Poggio La Caccia, each with distances in kilometers.](https://cdn-images-1.medium.com/max/1024/1*44L4h0-81kbAtkKGNa9Lgg.jpeg)
...
![Line chart titled “Chasing noise: a refit triggered by a one-off spike,” showing actual forecast data with a sharp spike at month 18, comparing a “Hold” model (smooth curve) against a “Refit on the spike” model (dashed line that overreacts to the spike) from months 19–35.](https://cdn-images-1.medium.com/max/1024/1*pKThQTOJUzt5Ve-SBwjYqg.png)
...
![Chart showing actual values rising steadily after a trend reversal, while repeated forecast projections consistently underestimate the new trend, resulting in a steadily growing, one-directional residual error.](https://cdn-images-1.medium.com/max/1024/1*wcA8mk-WyFyi-6Mx9bCisQ.png)
...
![Chart showing actual forecast data with a temporary spike (months 16–20), comparing a “widen the prior” approach that tracks actuals smoothly against a “full rebuild” approach (dashed red) that overshoots and drifts out of sync afterward.](https://cdn-images-1.medium.com/max/1024/1*iObLmsenle5sh-ZRmZTKPg.png)
...
![Decision flowchart for handling a missing forecast: checks if the error is noise (Hold), one-sided and worsening (Respecify), or otherwise (Refit).](https://cdn-images-1.medium.com/max/1024/1*nC-9XQFFP11ZdjenhyKEQw.png)
...
![Two charts: left shows two markets that moved together before a shock, then diverged into fast vs. slow recovery paths; right shows forecast error over time, with a ‘refit only’ model staying persistently high while a ‘respecified’ model converges back to the pre-shock baseline error.](https://cdn-images-1.medium.com/max/1024/1*vQdKuqWk1aU7FvgrRu96bg.png)
...
![](https://medium.com/_/stat?event=post.clientViewed&referrerSource=full_rss&postId=c606b9bdb0ab)

---

[How we knew COVID was over (and what our models had to unlearn)](https://medium.com/airbnb-engineering/how-we-knew-covid-was-over-and-what-our-models-had-to-unlearn-c606b9bdb0ab) was originally published in [The Airbnb Tech Blog](https://medium.com/airbnb-engineering) on Medium, where people are continuing the conversation by highlighting and responding to this story.
