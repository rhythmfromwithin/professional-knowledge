...
> 原文: [https://netflixtechblog.com/a-tale-of-two-flink-autoscalers-e9f6a1b1492b?source=rss----2615bd06b42e---4](https://netflixtechblog.com/a-tale-of-two-flink-autoscalers-e9f6a1b1492b?source=rss----2615bd06b42e---4)

[Samuel Yeboah](https://www.linkedin.com/in/samueltyeboah/), [Francesco Di Chiara](https://www.linkedin.com/in/dichiarafrancesco/) and [Mingliang Liu](https://www.linkedin.com/in/liuml07/)
...
When we started, the Flink community had no mature autoscaler to offer. By the time we re-evaluated, it did: the [Apache Flink Autoscaler](https://nightlies.apache.org/flink/flink-kubernetes-operator-docs-main/docs/custom-resource/autoscaler/). Instead of watching containers from outside, it reasons from inside the job.

![](https://cdn-images-1.medium.com/max/1024/1*Rn81hdaUHY94Sf8S-GOHZA.png)

Figure 1: Architecture of the two Flink autoscalers

Its key idea is to estimate each operator’s [true processing rate](https://www.usenix.org/conference/osdi18/presentation/kalavri) (TPR): the throughput it could sustain if it were fully busy. Flink reports, per subtask, the fraction of each second spent doing actual work, separate from time spent backpressured or idle. Dividing observed throughput by that busy fraction extrapolates capacity to full utilization: an operator handling 700 records/sec while busy 70% of the time has a TPR of 700 / 0.7 = 1,000 records/sec. Starting from the sources, the autoscaler walks the job graph and uses each operator’s TPR, its input/output ratios, and a target utilization to compute the parallelism every vertex needs so that no operator becomes the bottleneck, rather than resizing the whole cluster as a unit.

![](https://cdn-images-1.medium.com/max/1024/1*5vX9EEmUr1r5EDgWB8U8ew.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*nXaMxuXoCByEBitEs7iw-A.png)
...
![](https://cdn-images-1.medium.com/max/1024/1*ujsQFlUsGVC-qMycK4TjSw.png)
...
![](https://medium.com/_/stat?event=post.clientViewed&referrerSource=full_rss&postId=e9f6a1b1492b)

---

[A Tale of Two Flink Autoscalers](https://netflixtechblog.com/a-tale-of-two-flink-autoscalers-e9f6a1b1492b) was originally published in [Netflix TechBlog](https://netflixtechblog.com) on Medium, where people are continuing the conversation by highlighting and responding to this story.
